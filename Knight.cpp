...
// Used to return the knight symbol.
string Knight::getSymbol(bool pieceColour) {
    if (pieceColour == true) {
        return "♘";
    } else {
        return "♞";
...
// Checks if the knight is moving to a legal square
bool Knight::canMove(string *&Arr, bool colour, int startx, int starty, int endx, int endy) {
    ...
    // If the final position is occupied by a piece of the same colour, return false.
        ...
        int count = 0;
        for (int i=1; i<3; i++) {
            if (endx == startx + i && endy == starty + 3 - i) {
                count++;
            }
            else if (endx == startx - i && endy == starty + 3 - i) {
                count++;
            }
            else if (endx == startx + i && endy == starty - 3 + i) {
                count++;
